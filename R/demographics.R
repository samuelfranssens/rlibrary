...
demographics <- function(dataset){
  ...
  if ("subject" %nin% names(dataset) | "gender" %nin% names(dataset) | "age" %nin% names(dataset)){
    return(print("subject, gender, and age should be in your dataset"))
  } else {
    # n
    n <- length(dataset$subject)
    n.text <- paste("total number of participants:",n)

    # gender
    gender.ratio <- data.frame(table(dataset$gender))
    ...
    # age
    underage <- length(which(dataset$age < 18 & dataset$age!=0))
    noage    <- length(which(dataset$age==0))
    age <- dataset$age[dataset$age > 18]
